...
const toCode = rows.filter((r) => !r.prefilter);
const dir = rel("data/intake/packets");
fs.rmSync(dir, { recursive: true, force: true }); fs.mkdirSync(dir, { recursive: true });
const packets: string[] = [];
for (let i = 0; i < toCode.length; i += SIZE) {
  const name = `code-${String(Math.floor(i / SIZE) + 1).padStart(2, "0")}.json`;
...
// seeded 20 percent sample of prefiltered rejects, minimum 50, for coder B only
...
writeJson(rel("data/intake/packets", "sample-b.json"), { packet: "sample-b.json", note: "prefiltered rejects: coder B decides admission only", affiliations: Object.fromEntries(forecasters.map((f) => [f.slug, f.affiliations])), statements: sample });

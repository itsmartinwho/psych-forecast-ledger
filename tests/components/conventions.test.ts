...
const DIRS = ["components/layout", "components/card", "components/motion", "components/svg", "components/ui", "lib/testing"];
const FILES = [...DIRS.flatMap((d) => fs.readdirSync(path.join(ROOT, d)).map((f) => path.join(d, f))), "lib/format.ts", "app/(dev)/gallery/page.tsx"].filter((f) => /\.(ts|tsx)$/.test(f));
